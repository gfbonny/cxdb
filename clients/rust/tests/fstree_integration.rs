...
fn integration_fstree_snapshot_http() {
    ...
    let http_base = std::env::var("CXDB_TEST_HTTP_ADDR").unwrap_or_else(|_| "http://127.0.0.1:9010".to_string());
    ...
    std::fs::write(temp_dir.path().join("src").join("main.go"), "package main\n").unwrap();
    ...
    let readme = ureq::get(&format!("{http_base}/v1/turns/{}/fs/README.md", append.turn_id))
        .call()
    ...
    let append2 = client
        .append_turn(&ctx, &AppendRequest::new(head.context_id, TypeIDConversationItem, TypeVersionConversationItem, payload))
